...
================
*/
//Dan East: I had to add in Toggle Menu functionality, as seen in Quake I.
//This is a very useful func, I don't know why they removed it for Q2. :)
void M_ToggleMenu_f (void)
{
	...
	{
		if (m_menudepth)
			//A menu is being shown, so we pop it off
			M_PopMenu();
		else
			//No menu is being displayed, so display the main menu
			M_Menu_Main_f ();
		return;
	}
...
const char *M_Main_Key (int key)
{
	...
	case K_UPARROW:
		...
		return sound;
#ifdef _WIN32_WCE
	//Allow primary joy to select for CE
	case K_JOY1:
#endif
	case K_KP_ENTER:
	case K_ENTER:
...
#else
	M_DrawTextBox( 8, 120 - 48, 27, 3 );
	M_Print( 16 + 16, 120 - 48 + 8,  "Restarting the sound system.");
	M_Print( 16 + 16, 120 - 48 + 16, "This could take a minute," );
	M_Print( 16 + 16, 120 - 48 + 24, "so please be patient." );
#endif
...
void Options_MenuInit( void )
{
...
#else
		//Use shorter strings for CE version
		"max compat", "max perform", 0
#endif
	};
	...
	*/
	...
	s_options_cdvolume_box.generic.y		= y;
	...
	s_options_quality_list.generic.y		= y;;
	...
	s_options_compatibility_list.generic.y		= y;
#ifndef _WIN32_WCE
	s_options_compatibility_list.generic.name	= "sound compatibility";
#else
	s_options_compatibility_list.generic.name	= "sound compat";
#endif
	...
	s_options_alwaysrun_box.generic.y	= y;
	...
	s_options_invertmouse_box.generic.y	= y;
	...
	s_options_lookspring_box.generic.y	= y;
	...
	s_options_lookstrafe_box.generic.y	= y;
	...
	s_options_freelook_box.generic.y	= y;
	...
	s_options_crosshair_box.generic.y	= y;
...
*/
	...
	s_options_joystick_box.generic.y	= y;
	...
	s_options_customize_options_action.generic.y		= y;
	...
	s_options_defaults_action.generic.y		= y;
	...
	s_options_console_action.generic.y		= y;
...
#else
	//Allow primary joy to select for CE
	if ( key == K_ESCAPE || key == K_ENTER || key == K_JOY1)
#endif
	{
			...
			if ( !strstr( pcxnames[k], "_i.pcx" ) )
			{
				if ( IconOfSkinExists( pcxnames[k], pcxnames, npcxfiles - 1 ) )
				{
...
#else
					{
						...
						skinnames[s] = malloc(len+1);
						strcpy(skinnames[s], scratch);
					}
#endif
					s++;
				}
...
qboolean PlayerConfig_MenuInit( void )
{
	...
	qsort( s_pmi, s_numplayermodels, sizeof( s_pmi[0] ), (int (cdecl *)(const void *, const void *))pmicmpfnc );
...
const char *M_Quit_Key (int key)
{
	...
	case 'N':
		...
		break;
#ifdef _WIN32_WCE
	//For CE consider main joybutton as Yes.
	case K_JOY1:
#endif
	case 'Y':
	case 'y':
	...
	default:
#ifdef _WIN32_WCE
		//For CE let any other key be No
		M_PopMenu ();
#endif
		break;
	}
...
{
#ifdef _WIN32_WCE
	//Dan East: I had to add the togglemenu command back, which is found in Quake I.
	//This is neccessary to allow a single binding to both show, exit and hide menus.
