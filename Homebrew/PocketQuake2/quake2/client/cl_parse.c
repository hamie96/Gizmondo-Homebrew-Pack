...
//		Com_Printf ("100%%\n");
		...
		r = rename (oldn, newn);
#else
		//Dan East: Workaround for WinCE
		{
			TCHAR oldn_w[MAX_PATH];
			TCHAR newn_w[MAX_PATH];
			MultiByteToWideChar(CP_ACP, 0, oldn, -1, oldn_w, sizeof(oldn_w)/sizeof(TCHAR));
			MultiByteToWideChar(CP_ACP, 0, newn, -1, newn_w, sizeof(newn_w)/sizeof(TCHAR));
			r = !MoveFile(oldn_w, newn_w);
		}
#endif
		if (r)
