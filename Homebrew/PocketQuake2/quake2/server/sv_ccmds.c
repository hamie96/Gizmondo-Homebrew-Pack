...
#ifdef _WIN32_WCE
//Dan East:
//I had to use some alternate API functions for time, deleting files, etc, plus
//the win function for converting to UNICODE.
//I couldn't include winbase.h for those definitions because it conflicted with
//several of the names used by id, so I've just directly inserted what I needed below.
#define CP_ACP 0
...
extern int		MultiByteToWideChar(UINT uCodePage, DWORD dwFlags,
					LPCSTR lpMultiByteStr, int cchMultiByte,
					LPWSTR lpWideCharStr, int cchWideChar);
extern int		DeleteFileW(LPCTSTR lpFileName);
...
void SV_WipeSavegame (char *savename)
{
	...
	while (s)
	{
		...
		remove (s);
#else
	MultiByteToWideChar(CP_ACP, 0, s, -1, name_t, sizeof(name_t)/sizeof(TCHAR));
	DeleteFile(name_t);
#endif
		s = Sys_FindNext( 0, 0 );
	}
...
#else
		GetLocalTime(&st);
		Com_sprintf (comment,sizeof(comment), "%2i:%i%i %2i/%2i  ", st.wHour,
			st.wMinute/10, st.wMinute%10,
			st.wMonth+1, st.wDay);
#endif
